...
export class RideRepositoryDatabase implements RideRepository {
	...
	async get (rideId: string) {
		...
		if (!ride) return;
		return Ride.restore(ride.ride_id, ride.passenger_id, parseFloat(ride.from_lat), parseFloat(ride.from_long), parseFloat(ride.to_lat), parseFloat(ride.to_long), ride.status, ride.date, parseFloat(ride.last_lat), parseFloat(ride.last_long), parseFloat(ride.distance), parseFloat(ride.fare), ride.driver_id);
	}
	...
	async getActiveRidesByPassengerId (passengerId: string) {
		...
		for (const activeRideData of activeRidesData) {
			activeRides.push(Ride.restore(activeRideData.ride_id, activeRideData.passenger_id, parseFloat(activeRideData.from_lat), parseFloat(activeRideData.from_long), parseFloat(activeRideData.to_lat), parseFloat(activeRideData.to_long), activeRideData.status, activeRideData.date, parseFloat(activeRideData.last_lat), parseFloat(activeRideData.last_long), parseFloat(activeRideData.distance), parseFloat(activeRideData.fare), activeRideData.driver_id));
		}
		return activeRides;
	}
